...
func (r Running) Calories() float64 {
	...
	ms := r.meanSpeed()
	h := r.Duration.Hours()
	return ((CaloriesMeanSpeedMultiplier*ms + CaloriesMeanSpeedShift) * r.Weight / MInKm * h * MinInHours)
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	msMps := w.meanSpeed() * KmHInMsec
	hMeters := w.Height / CmInM
	minutes := w.Duration.Hours() * MinInHours
	return (CaloriesWeightMultiplier*w.Weight + ((msMps*msMps)/hMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * minutes
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool*s.CountPool) / MInKm / h
}
...
func (s Swimming) Calories() float64 {
	...
	ms := s.meanSpeed()
	h := s.Duration.Hours()
	return (ms + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * h
}
